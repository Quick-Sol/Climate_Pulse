/**
 * ClimaPulse — Navigation & shared layout
 *
 * Provides renderHeader(), renderFooter(), renderMobileMenu(), and helpers
 * to create consistent Lucide icons.
 */
...
const ICON_PATHS = {
  ...
  umbrella: `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 12a10.06 10.06 0 0 0-20 0Z"/><path d="M12 12v8a2 2 0 0 0 4 0"/><path d="M12 2v1"/></svg>`,
  ...
  "leaf-icon": `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 20A7 7 0 0 1 9.8 6.1C15.5 5 17 4.48 19 2c1 2 2 4.18 2 8 0 5.5-4.78 10-10 10Z"/><path d="M2 21c0-3 1.85-5.36 5.08-6C9.5 14.52 12 13 13 12"/></svg>`,
...
export function icon(name, size = 20) {
  const svg = ICON_PATHS[name] || ICON_PATHS.cloud;
...
/** Static icon markup string (for use inside template literals). */
export function iconMarkup(name, size = 20) {
  return (ICON_PATHS[name] || ICON_PATHS.cloud)
    .replace('width="24"', `width="${size}"`)
    .replace('height="24"', `height="${size}"`);
}

const NAV_ITEMS = [
  { href: "index.html", label: "Dashboard", icon: "cloud", match: ["index.html"] },
  { href: "gases.html", label: "Gas Intelligence", icon: "wind", match: ["gases.html", "gas-detail.html"] },
  { href: "soil.html", label: "Soil Intelligence", icon: "sprout", match: ["soil.html", "soil-detail.html"] },
  { href: "water.html", label: "Water Intelligence", icon: "droplets", match: ["water.html", "water-detail.html"] },
  { href: "government.html", label: "Government", icon: "landmark", match: ["government.html"] },
  { href: "industrial.html", label: "Industrial Zones", icon: "factory", match: ["industrial.html"] },
  { href: "crop-adviser.html", label: "Crop Adviser", icon: "leaf", match: ["crop-adviser.html"] },
  { href: "advisor.html", label: "Climate Advisor", icon: "sparkles", match: ["advisor.html"] },
];

export function activeNavPath() {
  const p = window.location.pathname.split("/").pop() || "index.html";
  for (const item of NAV_ITEMS) {
    if (item.match.includes(p)) return item.href;
  }
  return null;
}
...
export function renderHeader() {
  ...
  header.innerHTML = `
    <div class="container-max flex items-center justify-between h-16">
      <a href="index.html" class="flex items-center gap-3" aria-label="ClimaPulse home">
        <span class="logo-badge">${iconMarkup("cloud", 24)}</span>
        <span class="flex flex-col leading-tight">
          <span class="text-lg font-extrabold text-slate-900 tracking-tight">Clima<span class="text-gradient">Pulse</span></span>
          <span class="logo-subtitle text-[11px] font-medium text-slate-500 -mt-0.5">Climate Intelligence</span>
        </span>
      </a>

      <nav class="desktop-nav items-center gap-0.5" aria-label="Primary">
        ${NAV_ITEMS.map((n) => `
          <a href="${n.href}" class="nav-link ${active === n.href ? "active" : ""}" aria-current="${active === n.href ? "page" : "false"}">
            ${iconMarkup(n.icon, 18)}
            <span>${n.label}</span>
          </a>`).join("")}
      </nav>

      <button class="icon-btn mobile-menu-btn" id="mobile-menu-btn" aria-label="Open navigation menu" aria-expanded="false" aria-controls="mobile-menu">
        ${iconMarkup("menu", 22)}
      </button>
    </div>
  `;
...
export function renderFooter() {
  ...
  if (!footer) return;
  footer.className = "clima-footer mt-10";
  footer.innerHTML = `
    <div class="container-max py-8 flex flex-col sm:flex-row items-center justify-between gap-4">
      <div class="flex items-center gap-3">
        <span class="logo-badge" style="width:2.2rem;height:2.2rem;border-radius:0.625rem;">${iconMarkup("cloud", 20)}</span>
        <div>
          <p class="text-sm font-bold text-slate-800">© 2026 ClimaPulse</p>
          <p class="text-xs text-slate-500">Phase 1 data is simulated for demonstration purposes.</p>
        </div>
      </div>
      <div class="flex items-center gap-4 text-xs text-slate-500">
        <a href="index.html" class="hover:text-slate-800 hover:underline">Dashboard</a>
        <a href="government.html" class="hover:text-slate-800 hover:underline">Government</a>
        <a href="advisor.html" class="hover:text-slate-800 hover:underline">Climate Advisor</a>
      </div>
    </div>
  `;
}
...
function attachMobileMenu() {
  ...
  panel.className = "mobile-menu-panel flex flex-col";
  panel.innerHTML = `
    <div class="flex items-center justify-between mb-6">
      <div class="flex items-center gap-3">
        <span class="logo-badge" style="width:2.3rem;height:2.3rem;border-radius:0.625rem;">${iconMarkup("cloud", 22)}</span>
        <span class="font-extrabold text-slate-900 text-lg">Clima<span class="text-gradient">Pulse</span></span>
      </div>
      <button class="icon-btn" id="mobile-menu-close" aria-label="Close navigation menu">${iconMarkup("x", 22)}</button>
    </div>
    <nav class="flex flex-col gap-1" aria-label="Mobile primary">
      ${NAV_ITEMS.map((n) => `
        <a href="${n.href}" class="nav-link nav-link-full ${active === n.href ? "active" : ""}" aria-current="${active === n.href ? "page" : "false"}">
          ${iconMarkup(n.icon, 20)}
          <span>${n.label}</span>
        </a>`).join("")}
    </nav>
  `;
